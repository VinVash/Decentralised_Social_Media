...
contract SocialMedia is AutomationCompatibleInterface {
    ...
    uint256 public numLeaderboard; // number of users on the leaderboard.
        ...
        checkPostId(_postAuthor, _postId)
    {
        if (likes[_postId][msg.sender]) {
            ...
            for (uint256 i = 0; i < posts[_postId].likeAddress.length; i++) {
                if (posts[_postId].likeAddress[i] == msg.sender) {
                    delete posts[_postId].likeAddress[i];
                    break;
                }
            }
            for (uint256 i = 0; i < userPosts[_postAuthor].length; i++) {
                for (
                    uint256 j = 0;
    ...
    {
        scores[_postAuthor] -= newCommentScore;
        for (uint256 i = 0; i < posts[_postId].commentAddress.length; i++) {
    ...
    function getAllLeaderboard() public view returns (LeaderScore[] memory) {
        LeaderScore[] memory leaderB = new LeaderScore[](leaderboard.length);
    ...
    function censorPost(address postAuthor, uint256 postId)
        public
        onlyModerator
    {
        require(!posts[postId].isCensored, "Post is already censored.");
        posts[postId].banCounter++;
        for (uint256 i = 0; i < userPosts[postAuthor].length; i++) {
    ...
    function uncensorPost(address postAuthor, uint256 postId)
        public
        onlyModerator
    {
